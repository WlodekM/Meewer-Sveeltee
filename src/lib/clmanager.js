...
authHeader.subscribe(v => {
	...
	if (_authHeader.username && _authHeader.token) {
		localStorage.setItem("meewer_savedusername", _authHeader.username);
		localStorage.setItem("meewer_savedpassword", _authHeader.token);
	}
});
...
addEventListener("storage", event => {
	if (event.key === "meewer_savedusername") {
	...
	} else if (event.key === "meewer_savedpassword") {
...
if (localStorage.getItem("meewer_savedconfig")) {
	const savedConfig = JSON.parse(localStorage.getItem("meewer_savedconfig"));
...
/**
 * Send a "Meewer request" - a packet that makes the server respond with a direct and a statuscode packet.
 *
 * @param {object} data
 ...
 */
export async function meewerRequest(data) {
	link.log("manager", "meewer request", data);
	spinner.set(true);
 ...
 *
 * @returns {Promise<object | string>} Either an object or an error code; see meewerRequest.
 */
